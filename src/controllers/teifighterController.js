teifighterController = function($scope, $location, $timeout, teiService, lineService, questionService) {
  ...
  $scope.selectArea = function(area) {
    ...
    for (var i = 0; i < area.lines.length; i++) {
      ...
      if (line instanceof Line) {
        ...
        if (line.poly == null) {
          var segments = new Array(line.points.length);
          for (var i = 0; i < line.points.length; i++) {
            segments[i] = new Array(2);
            segments[i][0] = line.points[i].x;
            segments[i][1] = line.points[i].y;
          }

          line.poly = new paper.Path({
            segments: segments,
            strokeColor: 'black',
            fillColor: $scope.drawingOptions.line_color,
            strokeColor: 'black'
          });
          line.poly.fillColor.alpha = $scope.drawingOptions.opacity;
          line.poly.area = area;
          line.poly.onClick = function() {
            $scope.selectLine(this.area, this.line);
            $scope.update();
          };
        } else {
          area.lines[i].poly.visible = true;
        }
        console.log("POLYGON");
      }
  ...
  $scope.selectLine = function(area, line) {
    ...
    if (line instanceof Line) {
      ...
      line.poly.fillColor = $scope.drawingOptions.selected_line_color;
      line.poly.fillColor.alpha = $scope.drawingOptions.opacity;
    }
    //$scope.areaSelected = area;
  ...
  $scope.unselectCurrentLine = function() {
    if ($scope.lineSelected) {
      if ($scope.lineSelected instanceof Line) {
      ...
      } else if ($scope.lineSelected instanceof PolyLine) {
        $scope.lineSelected.poly.fillColor = $scope.drawingOptions.line_color;
        $scope.lineSelected.poly.fillColor.alpha = $scope.drawingOptions.opacity;
      }
    }
    $scope.lineSelected = null;
  }
  ...
  //Removes all rectangles and lines
  ...
  $scope.autoDetectHistLines = function() {
  ...
  // Line detection dealing
  $scope.autoDetectGaborLines = function() {
    var hasTranscription = false;
    for (var l = 0; l < $scope.areaSelected.lines.length; l++) {
      var line = $scope.areaSelected.lines[l];
      if (line.transcription) {
    ...
    var getLinesGabor = function() {
      //lineService.getAreaLines(
      lineService.getLinesGabor(
        $scope.currentUrl,
        $scope.areaSelected.top,
        $scope.areaSelected.left,
        $scope.areaSelected.bottom,
        ...
        $scope.areaSelected.linesFromPolygonList(result);
